...
test("writes planned MVP training session state fields", async () => {
  ...
  await repository.createSession({
    ...
    shouldFinish: false,
    lastPhaseAt: "2026-04-28T12:00:00.000Z",
  ...
  assert.match(executed[0] ?? "", /"completedSlideIds"/u);
  assert.match(executed[0] ?? "", /"currentFollowupCount"/u);
  assert.match(executed[0] ?? "", /"finalQuestionIndex"/u);
  assert.match(executed[0] ?? "", /"lastPhaseAt"/u);
...
test("writes training turn context fields including retrieved sources and speech", async () => {
  ...
  assert.match(executed[0] ?? "", /"turnType"/u);
  assert.match(executed[0] ?? "", /"phaseBefore"/u);
  assert.match(executed[0] ?? "", /"phaseAfter"/u);
  assert.match(executed[0] ?? "", /"slideFeedbackSummary"/u);
