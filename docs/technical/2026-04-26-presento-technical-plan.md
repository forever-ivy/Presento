...
Next.js Web App
  ...
  -> Defense Realtime Orchestrator 代理浏览器与 GLM-Realtime-Flash
  -> LangGraph.js 负责回合后分析、复盘、薄弱点和 deep-dive
  -> Vercel AI SDK 只负责文件讲解与文本流式交互
...
| SurfSense | https://github.com/Decentralised-AI/SurfSense-Open-Source-Alternative-to-NotebookLM | NotebookLM / Perplexity 类资料库、 cited answers、混合检索、层级索引、连接器 | 现已确定只并入其检索核心思路与兼容代码：hybrid retrieval、RRF、rerank、层级 scope；不引入其 users/search spaces/chats 主模型，按 Apache-2.0 保留 NOTICE / LICENSE |
...
- SurfSense 的检索核心已经进入实现范围：统一并入现有 `notebook-rag` sidecar，负责 hybrid search、RRF、rerank 和层级 scope。
...
| 向量检索 | pgvector + SurfSense-style retrieval core | 文档块、PPT 页、代码块、数据解释的向量召回、全文召回、RRF 融合与 rerank |
...
| 能力 | 推荐库 | 职责 |
|---|---|---|
| 流式交互 | Vercel AI SDK | 文件讲解和文本类交互的流式输出，不承担模拟答辩实时语音传输 |
| AI Chat UI | Vercel AI Elements | AI 老师消息流、输入框、响应区、操作按钮等基础组件 |
| 实时语音引擎 | GLM-Realtime-Flash | 实时语音会话、可打断老师追问、字幕和音频回包 |
| Realtime Orchestrator | 独立 Node/TypeScript 服务 | 浏览器 WebSocket、Provider WebSocket、事件转译、turn finalizer |
...
MVP 的正式技术路线不再是“录音文件 + TTS 播报”，而是：

> `GLM-Realtime-Flash` 负责实时双向语音会话；`Defense Realtime Orchestrator` 负责浏览器与 Provider 之间的双向 WebSocket、事件转译和回合收敛；`LangGraph.js` 只负责回合后结构化分析和整场复盘。

| 阶段 | 能力 | 实现方式 |
|---|---|---|
| MVP 正式版 | AI 老师实时语音追问 | 浏览器连接自建 realtime service，由 service 代理到 `GLM-Realtime-Flash` |
| MVP 正式版 | 实时字幕与老师语音回包 | provider 返回 transcript / audio delta；前端消费自定义事件协议 |
| MVP 正式版 | 用户文字输入兜底 | 在 realtime session 内允许 `input_text.submit`，用于弱网或无麦克风环境 |
| MVP 正式版 | 回合后结构化点评 | assistant response 完成后，由 turn finalizer 调 `current_slide_followup` 生成 score、risks、followUps、citations |
| MVP 正式版 | 训练结束复盘 | `review_report` 基于正式 `TrainingTurn` 生成 Review、Weakness、DeepDive |
| 第二阶段 | 语速、停顿、口头禅分析 | 在 realtime event store 基础上叠加音频分析 |
| 第二阶段 | 更强语音评测 | 引入表达节奏、停顿质量、抗压稳定性等专项评估 |

第一版强制做到：

- 浏览器不直连 GLM，不暴露 provider key。
- 模拟答辩只走 realtime session，不再以录音文件上传作为正式主链路。
- 支持实时老师语音、实时字幕、打断和文字 fallback。
- 每轮结束后落正式 `TrainingTurn`，再做结构化评分与追问建议。

第一版暂不强做：

- 复杂情绪识别。
- 口头禅自动矫正。
- 多人同时接入同一个 realtime session。
- 浏览器直连 provider。

`SpeechProvider` 抽象保留给其他场景或未来扩展，但模拟答辩正式链路不再依赖 MiniMax TTS。
...
flowchart TB
  ...
  Realtime["Defense Realtime Service\nNode/TypeScript + WebSocket"]
  ...
  AIGraph["AI Workflow Service\nLangGraph.js (post-turn analysis)"]
...
6. **Defense Realtime Service**
   - 负责浏览器实时连接、provider 连接、事件转译、打断、turn finalizer 和 session 生命周期。
   - 技术：Node.js、TypeScript、WebSocket、GLM-Realtime-Flash。

7. **AI Workflow Service**
   - 不再承担实时音频主循环，只负责回合后分析、复盘、钻研和内容转化。
   - 技术：LangGraph.js、LangChain Core。

8. **Agent Skills Runtime**
...
实现原则：优先复用 Open Notebook、SurfSense 等开源 NotebookLM 类项目中已经验证过的 sources、citations、chat sessions、content transformations、hybrid RAG 和 hierarchical indices 设计。其中 SurfSense 现已确定落地为“并入现有 `notebook-rag` sidecar 的检索核心”，而不是独立产品后端。只有 Presento 特有的答辩语境、速通 / 精通输出格式、薄弱点回流和逐页讲稿联动需要自研。
...
startTrainingSession
  ...
  -> browserConnectsToRealtimeService
  -> providerStreamsTranscriptAndAudio
  -> assistantInterruptIfNeeded
  -> finalizeTurnFromRealtimeEvents
  -> runCurrentSlideFollowupAsPostTurnAnalyzer
...
- 实时主循环必须交给 `GLM-Realtime-Flash`，不是普通 chat completions 拼接。
- 浏览器只能连接自建 realtime service，不直连 provider。
- 每轮 assistant response 完成后，必须先收敛成正式 `TrainingTurn`，再做结构化分析。
- `current_slide_followup` 在该架构中是回合后分析器，不是实时老师语音主循环。
- 第一版允许用户使用文字输入兜底。
- 第一版暂不强做语速分析、口头禅分析和情绪分析。
- 实时阶段给轻反馈，不每轮完整打分。
- 训练结束后统一生成复盘报告。
...
本阶段的 Runtime 只支持**官方内置 Skills**，不开放用户自定义、导入 manifest、可视化 workflow，也不做开放式 agent planning。调用模式固定为**业务侧显式 skillId 调用为主，Router 推荐为辅**。

它包含 5 个部分：

1. **Skill Registry**
   - 只注册平台内置 Skills。
   - 采用代码内建、版本化 manifest，不落数据库可编辑定义。
   - 记录 Skill id、version、名称、适用项目类型、触发场景、输入 schema、输出 schema、白名单工具、fallbackPolicy 和 traceTags。

2. **Skill Router**
   - 根据当前页面、用户动作、项目类型、文件类型、当前 PPT 页和训练状态推荐 Skill。
   - 主模式是显式 skillId 调用；`resolve` 只返回推荐结果和理由。
   - 例如用户在同屏答辩页点击“我讲完了”，推荐 `current_slide_followup`，并联动 `rubric_scoring`、`evidence_gap_check`、`member_scope_defense`。

3. **Skill Executor**
   - 负责执行 Skill。
   - 执行前用 Zod 校验输入，执行中调用 retriever / LLM / 受控工具，执行后再校验输出结构。
   - 所有业务链路统一通过 Executor，避免各条 route 直接拼 prompt 或直接调 graph。

4. **Tool Layer**
   - 提供受控工具，不让模型自由调用任意能力。
   - 本阶段白名单工具固定为：`retrieveProjectContext`、`retrieveSlideContext`、`retrieveKnowledgeNodeContext`、`retrieveFileContext`、`retrieveCodeContext`、`retrieveRiskQuestions`、`writeArtifact`、`writeReview`、`createWeakness`、`createDeepDive`、`writeContentExport`。
   - 禁止任意数据库访问、外部网络访问、执行用户代码。

5. **Skill Trace**
   - 每次 Skill 调用写入 Langfuse。
   - 同时保留本地 `SkillInvocation` 账本。
   - 记录 Skill id、version、resolvedBy、输入摘要、检索来源、工具调用、模型、输出摘要、用户反馈和失败原因。
...
#### 6.8.2 内置 Skill Packs

本阶段 Skill Packs 也采用代码内建 manifest，并支持项目级启用状态：
...
项目级只保存 Pack 启用状态与推荐日志，不开放自定义 Skill CRUD。
...
LangGraph State
  ...
  -> Persist Invocation / Artifact / Turn / Weakness / Export
  -> Langfuse Trace + Feedback Score
```
...
| `RealtimeSession` | 一次训练中的实时会话，支持断线重连和 provider session 映射 |
| `RealtimeEvent` | 实时事件账本，记录字幕、状态、provider 响应元数据和 turn 收敛输入 |
| `TrainingTurn` | 训练中的一轮问答 |
| `VoiceCapture` | 历史录音链路遗留表，不再作为正式训练主链路 |
| `Weakness` | 薄弱点 |
| `DeepDive` | 钻研结果 |
| `ReviewReport` | 训练复盘 |
...
TrainingSession 1 - N RealtimeSession
RealtimeSession 1 - N RealtimeEvent
TrainingSession 1 - N TrainingTurn
TrainingTurn N - 1 RealtimeSession
RealtimeEvent 0 - 1 TrainingTurn
...
/projects/[projectId]/reviews/[sessionId]
...
- `services/defense-realtime`：浏览器实时连接、provider bridge、event store、turn finalizer。
...
POST   /api/projects/:projectId/training-sessions/:sessionId/realtime-sessions
PATCH  /api/projects/:projectId/training-sessions/:sessionId/realtime-sessions/:realtimeSessionId/context
...
模拟答辩实时语音接口使用自建 realtime orchestrator 的 WebSocket 协议；Vercel AI SDK 保留给文件讲解和其他文本流式交互。后端仍需在每轮结束后持久化 `TrainingTurn`、检索来源和 trace id。
...
- `services/defense-realtime` 负责浏览器与 `GLM-Realtime-Flash` 的双向会话，不承载业务页面逻辑。
...
| 实时语音 Provider 波动 | 训练体验受损 | 浏览器不直连 provider；自建 realtime service 负责重试、事件转译和文字 fallback |
...
- 知识地图节点和边生成。
- 训练会话状态流转。
- realtime session / realtime event 状态机。
- turn finalizer、打断和文本 fallback。
- 权限隔离。
...
DEFENSE_REALTIME_PORT
DEFENSE_REALTIME_WS_URL
LLM_PROVIDER
LLM_API_KEY
EMBEDDING_MODEL
CHAT_MODEL_FAST
...
| Realtime Voice Engine | GLM-Realtime-Flash |
| Realtime Orchestrator | 独立 Node/TypeScript WebSocket 服务 |
| Workflow | LangGraph.js |
| Agent Skills | LangGraph.js + Zod + YAML / JSON manifest + Langfuse trace |
| RAG Base | LangChain Core + SurfSense-style hybrid retriever |
| Jobs | Trigger.dev，或 BullMQ + Redis |
| Observability | Langfuse + Sentry + PostHog |
| AI Eval | Promptfoo + Vitest |
| Voice MVP | GLM-Realtime-Flash + 自建 realtime orchestrator + 文本 fallback + 回合后 LangGraph 分析 |
| PCG | QQ / 微视 / 腾讯视频 mock service |
...
- GLM Realtime API: https://docs.bigmodel.cn/cn/asyncapi/realtime
